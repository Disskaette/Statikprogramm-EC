...
def spannungsverlauf_final_export():
    ...
    for i, row in df.iterrows():
        ...
        if sigma_i < 0:
            ...
            x_offset = get_text_width_in_data_coords(ax, label) * 1.1
            ax.text(0.1 * max_sigma, y, label, color="red",
                    fontsize=7, ha="left", va="center")

        elif sigma_i > 0:
            label = f"{sigma_i:.3f} N/mm²"
            x_offset = get_text_width_in_data_coords(ax, label) * 1.1
            ax.text(- (0.1 * max_sigma + x_offset), y, label,
    ...
    y_labels = []
    x_y_beschriftung = -(2.5 * max_sigma)
    for i, y in enumerate(df["y"]):
        y_label = ax.text(x_y_beschriftung, y, f"{y:.2f} m",
                          va="center", ha="right", fontsize=7)
        y_labels.append(y_label)

    # Länge und Pfeilkopf abhängig von max_sigma
    pfeil_laenge = max(0.5 * max_sigma, 0.05)  # min. 5 cm (Datenkoordinaten)
    # z. B. min. 0.02 N/mm² sichtbar
    kopf_laenge = max(0.1 * pfeil_laenge, 0.05)
    kopf_breite = 0.03 * \
        kopf_laenge if pfeil_laenge > 2 else max(0.02 * kopf_laenge, 0.05)

    print("Pfeillänge:", pfeil_laenge)
    print("Kopflänge:", kopf_laenge)
    print("Kopfbreite:", kopf_breite)
    ...
    if r_zug != 0 and y_schwerpunkt_zug:
        ...
        ax.text(startpunkt_pfeil_beschriftung_zug, y_schwerpunkt_zug,
                f"Rt = {format_kraft(r_zug_kn)} kN", color='blue', fontsize=7, ha="left")
    ...
    if r_druck != 0 and y_schwerpunkt_druck:
        ...
        ax.text(startpunkt_pfeil_beschriftung_druck, y_schwerpunkt_druck,
                f"Rc = {format_kraft(r_druck_kn)} kN", color='red', fontsize=7, ha="right")

    # Hilfslinien zur besseren Orientierung der Schwerpunkte
    # Beschriftung der y-Achse Schwerpunkte
    text_zug = f"{y_schwerpunkt_zug:.2f} m"
    text_druck = f"{y_schwerpunkt_druck:.2f} m"

    ax.text(x_y_beschriftung + 0.35*max_sigma, y_schwerpunkt_zug,
            text_zug,
            color='blue', fontsize=7, ha="right")
    ax.text(x_y_beschriftung + 0.35*max_sigma, y_schwerpunkt_druck,
            text_druck,
            color='red', fontsize=7, ha="right")

    # zeichenn der Hilfslinien
    x_start = -max_sigma_druck
    x_end = max_sigma_zug
    if y_schwerpunkt_zug:
        ax.hlines(y_schwerpunkt_zug, 0, x_end, color='blue',
                  linestyle='--', linewidth=0.6, alpha=0.6)

    if y_schwerpunkt_druck:
        ax.hlines(y_schwerpunkt_druck, x_start, 0, color='red',
                  linestyle='--', linewidth=0.6, alpha=0.6)
    ...
    import subprocess
    subprocess.run(["open", file_path])  # öffnet die Datei

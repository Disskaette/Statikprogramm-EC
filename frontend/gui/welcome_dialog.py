...
class WelcomeDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        ctk.CTkFrame(self, height=2, fg_color="#D7D7D7").pack(
            fill="x", pady=10)
        ...
        # Neues Projekt
        btn_new = ctk.CTkButton(button_frame,
        ...
        # Projekt öffnen
        btn_open = ctk.CTkButton(button_frame,
        ...
        btn_quick = ctk.CTkButton(button_frame,
        ...
        # Separator
        ctk.CTkFrame(button_frame, height=2).pack(fill="x", pady=15)

        # Recent Projects - Titel zentriert, gleiche Schriftgröße
        recent_label = ctk.CTkLabel(button_frame,
        ...
        self.recent_frame = ctk.CTkFrame(button_frame)
        ...
        ctk.CTkCheckBox(footer_frame,
                        text="Diesen Dialog beim Start anzeigen",
                        variable=self.show_again_var).pack(side="left")

    def _load_recent_projects(self):
        """Lädt und zeigt Recent Projects (mit Metadaten)"""
        # Hole Recent Projects mit automatischem Cleanup nicht existierender Projekte
        ...
        for project_entry in recent[:3]:
            # Neue Struktur: {uuid, path, name, last_opened}
            project_path = project_entry.get("path", "")
            project_name = project_entry.get("name", Path(project_path).name)
            last_opened = project_entry.get("last_opened", "unknown")

            # Formatiere last_opened-Datum (falls vorhanden)
            if last_opened != "unknown":
                try:
            ...
            btn = ctk.CTkButton(self.recent_frame,
                                text=f"📁 {project_name}",
                                command=lambda p=project_path: self._on_recent_project(
                                    p),
                                width=300)
            btn.pack(pady=3)

            # Pfad und Datum als Tooltip (klein) - zentriert
            tooltip_text = project_path
            if date_str:
                tooltip_text += f" (zuletzt geöffnet: {date_str})"
    ...
    def _close(self):
        ...
        self.settings_manager.set_show_welcome_screen(
            self.show_again_var.get())

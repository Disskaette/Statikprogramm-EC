...
class MethodeLastkombiGZG:
    ...
    def berechne_gzg_lastkombination(self) -> dict:
        ...
        if not lasten or e is None:
            logger.error("GZG-Lastkombination: Fehlende Lasten oder ungültiges Sprungmaß.")
            return {}
        ...
        for last in lasten:
            ...
            kdef = kmod_entry.kdef if hasattr(kmod_entry, 'kdef') else 0.8  # Fallback
        ...
        if g_summe > 0:
            ...
            logger.debug(f"GZG Debug: Erstellt G-Kombination: {gzg_kombis[header]}")
        ...
        if len([q for q in q_lasten if q["psi2"] > 0]) > 1:
            ...
            for q in q_lasten:
                if q["psi2"] > 0:
                    ...
                    formel_teile.append(f"{q['psi2']:.1f} \\cdot {q['lastfall'].upper()}_k = {q['psi2'] * q['wert_e']:.2f}")
                    header_teile.append(f"\\psi_2 \\cdot {q['lastfall'].upper()}_k")
            
            header = " + ".join(header_teile)
            formel = " + ".join(formel_teile) + f" = {qd_gesamt:.2f} \\,\\text{{kN/m}}"
        ...
        if gzg_kombis:
            massgebende_kombi = max(gzg_kombis.items(), key=lambda x: x[1]["wert"])
            ...
            logger.info(f"Maßgebende GZG-Kombination: {name}, qd = {gzg_kombis[name]['wert']:.2f} kN/m, kdef = {gzg_kombis[name]['kdef']:.2f}")
    ...
    def render_latex_to_image(self, latex_str: str) -> Image.Image:
        """Rendert LaTeX-String zu PIL-Image (identisch zu anderen Modulen)"""
        try:
            import matplotlib

...
class ProjectManager:
    ...
    def create_project(self, project_name: str, description: str = "") -> Path:
        ...
        if project_path.exists():
            ...
            raise FileExistsError(f"Projekt '{project_name}' existiert bereits")
    ...
    def open_project(self, project_path: Path) -> Dict[str, Any]:
        ...
        if not project_file.exists():
            raise FileNotFoundError(f"Projekt-Datei nicht gefunden: {project_file}")
            ...
            if item.is_dir():
                ...
                if project_file.exists():
                    try:
                        ...
                        logger.error(f"Fehler beim Lesen von {project_file}: {e}")
    ...
    def create_position(self, position_model: PositionModel, 
        ...
        with open(position_file, 'w', encoding='utf-8') as f:
            json.dump(position_model.to_dict(), f, indent=2, ensure_ascii=False)
    ...
    def load_position(self, position_file: Path) -> PositionModel:
        ...
        if not position_file.exists():
            raise FileNotFoundError(f"Position nicht gefunden: {position_file}")
            ...
            try:
                with open(json_file, 'r', encoding='utf-8') as f:
                    ...
                    if "position_nummer" in data or "position_name" in data:
                        ...
                        data['relative_path'] = str(json_file.relative_to(self.current_project_path))
    ...
    def delete_position(self, position_file: Path):
        ...
        if position_file.exists():
            ...
            if self.current_project_path and self.current_project_data:
                relative_path = str(position_file.relative_to(self.current_project_path))
                if relative_path in self.current_project_data["positions"]:
                    self.current_project_data["positions"].remove(relative_path)
                    self.save_project()

...
# df = pd.read.excel("datenbank_dlt.xlsx", sheet_name="materials")
...
class Eingabemaske:
    ...
    def querschnitt_eingabe(self):
        ...
        self.querschnitt_frame.grid_columnconfigure(0,  minsize=200)
        self.querschnitt_frame.grid_columnconfigure(1,  minsize=100)

        self.materialgruppe_var = tk.StringVar(value="Vollholz")
        ttk.Label(self.querschnitt_frame, text="Festigkeit:", width=10).grid(
            row=0, column=0, sticky="w",)
        qs_materialgruppe_combo = ttk.Combobox(self.querschnitt_frame, textvariable=self.materialgruppe_var,
                                               values=["Vollholz", "BSP-Platte"], state="readonly", width=10)
        qs_materialgruppe_combo.grid(row=0, column=1, sticky="w")
        ...
        self.querschnitt_balken_frame = ttk.Frame(self.querschnitt_frame)
        self.querschnitt_balken_frame.grid(
            row=2, column=0, columnspan=5, sticky="w", padx=0, pady=10)
        self.querschnitt_balken_frame.grid_columnconfigure(0, minsize=200)
        self.querschnitt_balken_frame.grid_columnconfigure(1, minsize=100)

        # self.querschnitt_balken_frame.grid_columnconfigure(0, minsize=155)

        # # Container für BSP-Anzeige
        # self.querschnitt_balken_frame = ttk.Frame(self.querschnitt_frame)
        # self.querschnitt_balken_frame.grid(
    ...
    def update_querschnitt_felder(self):
        for widget in self.querschnitt_balken_frame.winfo_children():
            widget.destroy()

        if self.materialgruppe_var.get() == "Vollholz":
            self.querschnitt_var = tk.StringVar(value="Vollholz")
            ttk.Label(self.querschnitt_balken_frame, text="Typ:", width=10).grid(
                row=0, column=0, sticky="w")
            qs_typ_combo = ttk.Combobox(self.querschnitt_balken_frame, textvariable=self.querschnitt_var,
                                        values=["Vollholz", "BSP-Platte"], width=10, state="readonly")
            qs_typ_combo.grid(row=0, column=1, sticky="w")
            # qs_typ_combo.bind("<<ComboboxSelected>>",
            qs_typ_combo.bind("<<ComboboxSelected>>",
                              lambda e: self.update_querschnitt_felder())
            # Festigkeitsklasse
            ttk.Label(self.querschnitt_balken_frame, text="Festigkeit:", width=10).grid(
                row=1, column=0, sticky="w")
            self.festigkeitsklasse_vh = tk.StringVar(value="C24")
            qs_festigkeit_combo = ttk.Combobox(self.querschnitt_balken_frame, textvariable=self.festigkeitsklasse_vh,
                                               values=["C24", "C30"], width=10, state="readonly")
            qs_festigkeit_combo.grid(row=1, column=1, columnspan=2, sticky="w")
            qs_festigkeit_combo.bind("<<ComboboxSelected>>",
                                     lambda e: self.update_querschnitt_felder())
            ...
            ttk.Checkbutton(self.querschnitt_balken_frame, text="Variante 1", variable=self.checkbox_var_1,
                            ).grid(row=2, column=1, columnspan=2, sticky="w")
            ttk.Checkbutton(self.querschnitt_balken_frame, text="Variante 2", variable=self.checkbox_var_2,
                            ).grid(row=2, column=3, columnspan=2,  sticky="w")
            ttk.Checkbutton(self.querschnitt_balken_frame, text="Variante 3", variable=self.checkbox_var_3,
                            ).grid(row=2, column=5, columnspan=2, sticky="w")

            # Variante 1
            ttk.Label(self.querschnitt_balken_frame, text="b [mm]:").grid(
                row=3, column=1, sticky="w")
            self.b_entry_1 = ttk.Entry(self.querschnitt_balken_frame, width=5)
            self.b_entry_1.insert(0, "160")
            self.b_entry_1.grid(row=3, column=2, padx=5)

            ttk.Label(self.querschnitt_balken_frame, text="h [mm]:").grid(
                row=4, column=1, sticky="w")
            self.h_entry_1 = ttk.Entry(self.querschnitt_balken_frame, width=5)
            self.h_entry_1.insert(0, "400")
            self.h_entry_1.grid(row=4, column=2, padx=5)

            # # Variante 2
            # ttk.Label(self.querschnitt_balken_frame, text="b [mm]:").grid(
            #     row=2, column=2, sticky="w")
            # self.b_entry_2 = ttk.Entry(self.querschnitt_balken_frame, width=5)
            # self.b_entry_2.insert(0, "160")
            # self.b_entry_2.grid(row=2, column=3, padx=5)

            # ttk.Label(self.querschnitt_balken_frame, text="h [mm]:").grid(
            #     row=3, column=2, sticky="w")
            # self.h_entry_2 = ttk.Entry(self.querschnitt_balken_frame, width=5)
            # self.h_entry_2.insert(0, "400")
            # self.h_entry_2.grid(row=3, column=3, padx=5)

            # # Variante 3
            # ttk.Label(self.querschnitt_balken_frame, text="b [mm]:").grid(
            #     row=2, column=4, sticky="w")
            # self.b_entry_3 = ttk.Entry(self.querschnitt_balken_frame, width=5)
            # self.b_entry_3.insert(0, "160")
            # self.b_entry_3.grid(row=2, column=5, padx=5)

            # ttk.Label(self.querschnitt_balken_frame, text="h [mm]:").grid(
            #     row=3, column=4, sticky="w")
            # self.h_entry_3 = ttk.Entry(self.querschnitt_balken_frame, width=5)
            # self.h_entry_3.insert(0, "400")
            # self.h_entry_3.grid(row=3, column=5, padx=5)

        else:
            ttk.Label(self.querschnitt_balken_frame, text="Schichten [n]:").grid(
                row=0, column=0, sticky="w")
            self.bsp_schichten = ttk.Entry(
                self.querschnitt_balken_frame, width=5)
            self.bsp_schichten.insert(0, "5")
            self.bsp_schichten.grid(row=0, column=1)

            ttk.Label(self.querschnitt_balken_frame,
                      text="E-Modul [N/mm²]:").grid(row=0, column=2, sticky="w")
            self.bsp_emodul = ttk.Entry(
                self.querschnitt_balken_frame, width=8)

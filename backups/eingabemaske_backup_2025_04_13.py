...
class Eingabemaske:
    def __init__(self, root):
        ...
        def begrenze_fenstergroesse():
            ...
            # 🛠️ Content-Frame-Breite dem tatsächlichen Inhalt geben (für horizontales Scrollen)
            self.canvas.itemconfig(self.inner_window, width=actual_w)
    ...
    def setup_gui(self):
        ...
        ttk.Label(frame_system_eingabe,
                  text="⬅️ EXTREM LANGE ZEILE ➡️ " * 20).grid(row=0, column=0)
    ...
    def add_lasteneingabe(self):
        ...
        kommentar_entry.bind(
            "<KeyRelease>", self.update_lasten_memory, self.update_querschnitt_memory)
        lf_combo.bind("<<ComboboxSelected>>",
    ...
    def update_lasten_memory(self, event=None):
        ...
        for eintrag in self.lasten_eingaben:
            try:
                ...
                self.nkl = int(nkl_string.strip()[-1])
                ...
                kmod_entry = self.db.get_kmod(typ, self.nkl)
    ...
    def update_querschnitt_felder(self):
        ...
        if materialgruppe_1 == "Balken":
            ...
            qs_typ_combo_1.bind("<<ComboboxSelected>>",
                                lambda e: self.update_festigkeitsklasse_dropdown(), self.update_querschnitt_memory())
            ...
            self.qs_festigkeit_combo_1.bind(
                "<<ComboboxSelected>>", lambda e: self.update_querschnitt_memory())
            ...
            self.b_entry_1.bind("<KeyRelease>",
                                lambda event: self.update_querschnitt_memory(event))
    ...
    def gebrauchstauglichkeit_eingabe(self):
        ...
        situation_combo.bind("<<ComboboxSelected>>",
                             lambda e: self.update_gebrauchstauglichkeit_eingabe(), self.update_lasten_memory())
    ...
    def update_gebrauchstauglichkeit_eingabe(self):
        ...
        if self.situation_var.get() == "Allgemein":
            ...
            self.w_c_überhoehung.bind("<KeyRelease>",
                                      lambda event: self.update_lasten_memory(event))
    ...
    def update_querschnitt_memory(self, event=None):
        if not hasattr(self, "situation_var") or self.situation_var is None:
            return
        ...
        for eintrag in self.lasten_eingaben:
            try:
                ...
                materialgruppe_dic = self.db.get_materialgruppen()
                typ = getattr(
                    self, f"querschnitt_var_{aktuelle_variante}").get()
                ...
                #     materialgruppe_dic, typ, festigkeitsklasse, self.nkl_eingabe, self.kmod_typ
                # )

   def erstelle_feebb_dicts(self):
    ...
    def erstelle_feebb_dicts(self):
        ...
        # Kragarm links
        if self.eingabemaske.kragarm_links.get() and len(lager_knoten) > 1:
            # Lager ganz links deaktivieren
            supports[lager_knoten[0]] = [0, 0]
            # Letzter Knoten im Kragarm → Einspannung
            supports[lager_knoten[1] + 1] = [-1, -1]
            # Erster Knoten im Hauptfeld → auch Einspannung!
            supports[lager_knoten[1]] = [-1, -1]
            print("🏠 Kragarm links")

       # Kragarm rechts
        if self.eingabemaske.kragarm_rechts.get() and len(lager_knoten) > 1:
            # Letztes Lager deaktivieren
            supports[lager_knoten[-1]] = [0, 0]
            # Erster Knoten im Kragarm (rechts) → Einspannung
            supports[lager_knoten[-2] + 1] = [-1, -1]
            # Letzter Knoten im letzten Hauptfeld → auch Einspannung
            # supports[lager_knoten[-2]] = [-1, 0]
            print("🏠 Kragarm rechts")
        ...
        supports = [supports_flat[i:i+2]
                    for i in range(0, len(supports_flat), 2)]
        # Umwandlung von supports_flat in strukturierte Liste
        supports = [supports_flat[i:i+2]
                    for i in range(0, len(supports_flat), 2)]

        # Debug-Ausgabe aller Knotenlagerungen

...
class Eingabemaske:
    ...
    def _on_service_done(self, result=None, errors=None):
        ...
        def handle():
            ...
            self.kombi_anzeiger.aktualisiere_darstellung_threaded(result)
            # self.feebb.update_feebb_threaded(result)
    ...
    def get_spannweiten_dict(self) -> dict:
        ...
        spannweiten = {}
        for i, entry in enumerate(self.spannweiten_eingaben, start=1):
            try:
                spannweiten[f"l{i}"] = float(entry.get().replace(",", "."))
            except:
                pass
        return spannweiten
